...
class LoadFromFile (argparse.Action): #this is a custom action overriding the call method
    """
...
class ModelArgs:
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--hidden_dim', type=int, default=128, help='Number of hidden dimensions for RNN')
        self.parser.add_argument('--num_layers', type=int, default=2, help='Number of layers for RNN')
        # self.parser.add_argument('--dropout', type=float, default=0.2, help='dropout for RNN')
        self.parser.add_argument('--num_workers', type=int, default=0, help='num_workers in the dataloader')
        self.parser.add_argument('--data_batches', type=int, default=256, help='how many batches should the test/pred/gen data should run through')
        ...
        self.parser.add_argument('--P', type=ast.literal_eval, help='a string chaining train-val-gen student numbers to sample for customized number of students for either fixed or varying len data')
        # 'Number of unique instances,P*T must <=len(data_source) or len(dataset)'
        ...
        self.parser.add_argument('--mini_batch', type=str2bool, default=False, help='Use mini-batching for training., if enabled, other trainng methods  (hensman, standard_training,variational_inference_training ) need to be false')
        self.parser.add_argument('--hensman', type=str2bool, default=False, help='Use true mini-batch training.if enabled, other trainng methods (mini_batch, standard_training,variational_inference_training ) need to be false, when using hensman, has to use kl type of GPAprroxClosed')
        self.parser.add_argument('--variational_inference_training', type=str2bool, default=False, help='Use variational inference training.if enabled, other trainng methods  (mini_batch, standard_training,hensman ) need to be false')
        ...
        self.parser.add_argument('--marker', type=str, help='marker to differentiate data set, need to contain the domain dataset name you use to save the data_readings, can be in the format of domain_v1 or just domain', default=None)
        self.parser.add_argument('--ref_df_path', type=str, help='a csv file path had the whole data reading in data frame format to get the min, max of all the variables so we can inverse transform back',default=None)
        self.parser.add_argument('--training', type=str2bool, default=True, help='do we need to train or not')
...
class VAEArgs:
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--csv_file_test_label', type=str, help='Name of test label file', required=False)
        self.parser.add_argument('--test_mask_file', type=str, help='Name of test mask file', default=None)
        self.parser.add_argument('--marker', type=str, help='marker to differentiate data set', default=None)
        self.parser.add_argument('--dataset_type', required=False, choices=['RotatedMNIST', 'HealthMNIST', 'Physionet'],
                                 help='Type of dataset being used.')
        self.parser.add_argument('--num_workers', type=int, default=0, help='num_workers in the dataloader')
        self.parser.add_argument('--latent_dim', type=int, default=2, help='Number of latent dimensions')
        ...
        self.parser.add_argument('--run_tests', type=str2bool, default=False,
                                 help='Perform tests using the trained model')
        self.parser.add_argument('--f', type=open, action=LoadFromFile)
        self.parser.add_argument('--save_cadence', type=int, default=25, help='at what epoch number to save')
        self.parser.add_argument('--results_path', type=str,  help='directory to save generated data')
        self.parser.add_argument('--ref_df_path', type=str,  help='two ref path containing the data reading path and label data path threaded with "-"')
        self.parser.add_argument('--csv_file_val_data', type=str, help='Name of val data file', required=False)
        self.parser.add_argument('--batch_size', type=int, default=128, help='Number of samples fed into each batch')
        self.parser.add_argument('--model_params', type=str, default='model_params.pth',
                                 help='Pre-trained VAE parameters')
        self.parser.add_argument('--training', type=str2bool, default=False, help='disable training')
