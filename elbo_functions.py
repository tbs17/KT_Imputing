...
def deviance_upper_bound(covar_module0, covar_module1, likelihood, train_xt, m, log_v, z, P, T, eps):
    ...
    v = torch.exp(log_v)
    torch_dtype = torch.double
    print(f'train_xt.shape:{train_xt.shape}')
...
def minibatch_KLD_upper_bound(type_nnet,covar_module0, covar_module1, likelihood, latent_dim, 
m, H, train_xt, mu, log_v, z, P_tot, P_batch, T, natural_gradient, eps):
    """
    ...
    M = H.shape[-1] 
    # print(f'H/M (H.shape[-1]) shape:{H.shape}/{M}')
    # print(f'train_xt/P_batch/T/train_xt.shape[1].shape:{train_xt.shape}/{P_batch}/{T}/{train_xt.shape[1]}')
    if type_nnet=='rnn':
        T=1
    
    x_st = torch.reshape(train_xt, [P_batch, T, train_xt.shape[1]])
    stacked_x_st = torch.stack([x_st for i in range(latent_dim)], dim=1)
    # print(f'stacked_x_st.shape:{stacked_x_st.shape}')#stacked_x_st.shape:torch.Size([32, 64, 20, 9]), p_batch* Lat_D*T*dim
    # print(f'x_st/stacked_x_st/:{x_st.shape}/{stacked_x_st.shape}\n'
    # f'train_xt/z.shape:{train_xt.shape}/{z.shape}')#in simple mode, returns train_xt:[20,9],z:[32,60,9]
    # in lstm mode, train_xt:[20,20,9] and z is [32,60,20,9]
    ...
    K0zz = K0zz + eps * torch.eye(M, dtype=torch.double).to(device)#<---M->2*m
    # print(f'K0zz eigenvalues {torch.linalg.eig(K0zz)[0]} with shape:{K0zz.shape}')#torch.size(32,60,60)
    ...
    if type_nnet=='rnn':
        K0xz_st = torch.reshape(K0xz, [latent_dim, P_batch, 1, M])
    else:
    ...
    # print(f'K0xz/iK0zz/m/mu.T shape:{K0xz.shape}/{iK0zz.shape}/{m.shape}/{mu.T.shape}')
    if type_nnet=='rnn':
        # print(f'mu after NN shape:{mu.shape}')
        mu=mu.mean(0)
        # print(f'mu after NN shape after mean:{mu.shape}')
        # print(f'log_var after NN shape:{log_v.shape}')
        log_v=log_v.mean(0)
        # print(f'log_var after NN shape after mean:{log_v.shape}')
 
    # print(f'K0xz, iK0zz ,m shape:{K0xz.shape},{iK0zz.shape},{m.shape}') #K0xz, iK0zz shape:torch.Size([64, 50, 60]),torch.Size([64, 60, 60])
    # print(f'mu/log_v shape:{mu.shape}/{log_v.shape}')
    # print(f'iB_st shape:{iB_st.shape}\n'
    # f'/LB_st.shape: {LB_st.shape}\n'
    # f'K0_st{K0_st.shape}\n'
    # f'/K0zx_iB_K0xz{K0zx_iB_K0xz.shape}\n'
    # f'/H{H.shape}\n'
    # f'/LK0zz{LK0zz.shape}\n'
    # f'LH shape:{LH.shape}')
    c=torch.matmul(torch.matmul(K0xz, iK0zz), m).squeeze()
    d=mu.T
    # if type_nnet=='rnn':
    #     d=torch.stack([d for i in range(T)],dim=1).reshape(latent_dim,-1)
    _ = (c-d).reshape(latent_dim, P_batch, T, -1)
 
    A = torch.matmul(torch.matmul(_.transpose(2,3), iB_st), _).sum()
    e=log_v.T
    # if type_nnet=='rnn':
    #     e=torch.stack([e for i in range(T)],dim=1).reshape(latent_dim,-1)
    B = torch.sum(torch.diagonal(iB_st, dim1=-1, dim2=-2).reshape(latent_dim, -1)*torch.exp(e))
    ...
    if natural_gradient:
        ...
        # if type_nnet=='rnn':
        #     d=torch.stack([d for i in range(T)],dim=1).reshape(latent_dim,-1)
        mu = d.reshape(latent_dim, P_batch, T, -1)
...
def minibatch_KLD_upper_bound_iter(type_nnet,covar_module0, covar_module1, likelihood, latent_dim, m, H, train_xt, mu, log_v, z, P, P_in_current_batch, N, natural_gradient, id_covariate, eps):
    """
    Efficient unbiased estimate of the KL-divergence upper bound that enables the use of mini-batching.
    See L-VAE supplementary material.
