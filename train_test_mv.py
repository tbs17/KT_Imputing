...
def seed_model_runner(params):
    ...
    src_seqL=params['src_seqL']#<---
    tar_seqL=params['tar_seqL']#<---
    target_var_pos=params['target_var_pos']#<----
    pred_type=params['pred_type']#<_--
    tag_pos=params['tag_pos']#<--
    start=params['start']
    ...
    for i in range(2,6):
        ...
        if variate_type=='sv':
            ...
            results=sv_best_model_retriever(params)
            ...
            results=mv_best_model_retriever(params)
        
        results['data']=domain_src
        results['variate_type']=variate_type
        results['seed']=seed
        results['run_type']='model_run'
        results['lag_type']=lag_type
        df=df.append(results)
        display(df.head())
        out_dir='TRANS_MODEL/MODEL_RUN'
        os.makedirs(out_dir,exist_ok=True)
        df.to_csv(f'{out_dir}/{variate_type}_{domain_src}_{model_name}_{start_shift}_{total_time_step}_{na_fill_type}_seed_model_run.csv',index=False)
    print(f'obtain {df.shape[0]} models')
    display(df.head())
    return df
...
def time_feature_combo_run_old(data_params,train_params,model_params):
    from itertools import combinations
    from ast import literal_eval
    ...
    # test_feature_list=['seq_number', 'assessment_duration', 'bktcount', 'count',
    ...
    for j in shift_step_list:
        ...
        if training:
            if need_convert:
                feat_arr,label_arr, label_reg_arr,feat_lens,_=convert_npz_lagged_bkt_shift_mask(save_dir,data,na_fill_type,seq_size,j, remove_feature_list, test_feature_list,domain_src,stu_num,marker,model_transform,save_data_flag)
                if save_dir:
                    return feat_arr,label_arr, label_reg_arr,feat_lens,_
            else:
                assert need_convert==False, 'need_convert flag needs to be false, otherwise, will not load'
                data=np.load(converted_data_path,allow_pickle=True)
                feat_arr=data['feature']
                label_arr=data['target_id']
                label_reg_arr=data['proficiency']
            ...
            print(f'feat_arr.shape:{feat_arr.shape}')
            print(f'label_arr.shape: {label_arr.shape}')
            print(f'label_reg_arr.shape: {label_reg_arr.shape}')
            # print(f'feat_lens.shape: {len(feat_lens)}')
            is_predict=False
            train_loader,val_loader,test_loader=get_mv_data_mask(is_predict,na_fill_type,feat_lens,feat,label_arr, label_reg_arr,  batch_size,NUM_WORKER)
            dataloaders={
                'train':train_loader,
                'val':val_loader,
                'test':test_loader
...
# -------------below is mv non-transfer version trainer ---------------------
...
#             print(device)
            reg_pred = model(input)
            train_loss = criterion(reg_pred.to(device), label_reg) #the loss function is nn.BCEWithLogitsLoss(reduction='mean') which has built-in sigmoid function and average all the logitis already in it, so it can be compared with a float format of the target label_reg
...
# +++++++++BIG CHANGE: REMOVE DATA FROM 2018 ( only contains 108 rows and 8 users, these users don't have data in the 2019)++++++++++


def mv_best_model_retriever(params):
    seed=params['seed']
    start0=time.time()
    start=params['start']
    end=params['end']
    ...
    data_path=params['data_path']
    root_dir=params['root_dir']
    # data_path=f'TRANS_DATA/KT_data_2017-07-01_2019-06-30_{data_name}_eng_2019.csv'
    print(f'data_path: {data_path}')
    ...
    log_file_name='run_best'
    out_file_name=f'{data_name}_{model_name}_{na_fill_type}'
    # domain_src=f'{data_name}_bkt'
    out_dir=f'{data_name}_{model_name}_time_feat_combo'
    #     save_dir=os.path.join('TRANS_MODEL/BKT_MV',out_dir)
    ...
    time_feat_combo_run=time_feature_combo_run(data_params,train_params,model_params,start,end)
...
def time_feature_combo_run(data_params,train_params,model_params,start,end):
    from itertools import combinations
    from ast import literal_eval
    data=data_params['data']
    seq_size=data_params['seq_size']
...
#             if j%30==0:
            ...
            if model_name in ['lstm','LSTM','Lstm']:
                ...
                print(f'this is input=input[:,:]adaRNN model,you are training with {model_name} model')
                model=AdaRNN_noTL(na_fill_type,use_bottleneck=True, bottleneck_width=64, n_input=input_dim, n_hiddens=n_hiddens,  n_output=class_num, dropout=dropout, len_seq=tar_seqL, model_type=model_name).to(device)
            else:
                print(f'this is transformer model,you are training with {model_name} model')
                model = Transformer(na_fill_type,input_dim, hidden_dim, output_dim, q, v, h, N, attention_size=attention_size,
            ...
            checkpoint_format=f'TRANS_MODEL/BKT_MV/{out_dir}/procNum{start}-{end}/stuNUm_{stu_num}_feature{feature_len}_shiftStep{j}_SEED{seed}_{model_name}_{domain_src}_in{input_dim}_hid{hidden_dim}_{num_layers}numLayers_lr{learning_rate}_bs{batch_size}_seqL{src_seqL}_dw{weight_decay}_EP{n_epochs}_{na_fill_type}'
            direct_folder=checkpoint_format.split('/stuNUm')[0]
            ...
            best_epo_path,best_loss,best_loss_r,best_loss_l1=train_test_reg_mv(log_file, dataloaders, model, optimizer,checkpoint_format,data_params,train_params,model_params)
            new_df=pd.DataFrame({'model_name':[model_name],'combo_index':[i],'features':[subset],'shift':[j],'na_fill_type':[na_fill_type],
                                'ckpt':[best_epo_path],'MSE':[best_loss],'RMSE':[best_loss_r],'L1':[best_loss_l1]})
            df=df.append(new_df)
            # if j%5==0:
...
def stepUp_feature_run(data_params,train_params,model_params):
    from itertools import combinations
    from ast import literal_eval
    data=data_params['data']
    seq_size=data_params['seq_size']
    shift_step_list=data_params['shift_step_list']
    remove_feature_list=data_params['remove_feature_list']
    test_features=data_params['test_features']
    course_name=data_params['course_name']
    stu_num=data_params['num_stu']
    marker=data_params['marker']
    model_transform=data_params['model_transform']
    ...
    NUM_WORKER = data_params['NUM_WORKER']
    batch_size = data_params['batch_size']
    combo_file=data_params['combo_file']
    start_feature=data_params['start_feature']
    na_fill_type=data_params['na_fill_type']
    gpu_core=int(train_params['gpu_core'])
    ...
    if start_feature:
        ...
        start_index=dat.index[dat['combo']==start_feature].tolist()[0]
        print(f'start feature {start_feature} has index: {start_index}')
        run_df=dat.iloc[start_index:,]
        print(f'.....there are {run_df.shape[0]} features to go!!....')
    ...
    for i in run_df.index:
        ...
        for j in shift_step_list:
            ...
            feat_arr,label_arr, label_reg_arr=convert_npz_lagged_bkt_shift_mask(data,na_fill_type,seq_size,j, remove_feature_list, test_feature_list,course_name,stu_num,marker,model_transform,save_flag)
            ...
            train_loader,val_loader,test_loader=get_mv_data_mask(feat,label_arr, label_reg_arr,  batch_size,NUM_WORKER)
            dataloaders={
                'train':train_loader,
                'val':val_loader,
                'test':test_loader
            ...
            domain_src=train_params['domain_src']
            out_dir=train_params['out_dir']
            
            log_file_name=train_params['log_file_name']
            out_file_name=train_params['out_file_name']
            learning_rate = train_params['learning_rate']
            n_epochs = train_params['n_epochs']
            weight_decay=train_params['weight_decay']
            warm_up_step_count=train_params['warm_up_step_count']
            ...
            if model_name in ['lstm','LSTM','Lstm']:
                model=LSTM(na_fill_type,input_dim, hidden_dim, num_layers, output_dim).to(device)
            elif model_name in ['adaRNN','adaRNN','AdaRNN']:
                model=AdaRNN_noTL(na_fill_type,use_bottleneck=True, bottleneck_width=64, n_input=input_dim, n_hiddens=n_hiddens,  n_output=class_num, dropout=dropout, len_seq=tar_seqL, model_type=model_name).to(device)
            else:
                model = Transformer(na_fill_type,input_dim, hidden_dim, output_dim, q, v, h, N, attention_size=attention_size,
            ...
            checkpoint_format=f'TRANS_MODEL/BKT_MV/{out_dir}/stuNUm_{stu_num}_feature{test_feature_list}_shiftStep{j}_SEED{seed}_{model_name}_{domain_src}_in{input_dim}_hid{hidden_dim}_{num_layers}numLayers_lr{learning_rate}_bs{batch_size}_seqL{src_seqL}_dw{weight_decay}_EP{n_epochs}'
            direct_folder=checkpoint_format.split('/stuNUm')[0]
            os.makedirs(direct_folder,exist_ok=True)
            if save_flag:
            ...
            pprint(log_file,f'>>>>train model for {checkpoint_format}<<<<<<')

            best_epo_path,best_loss,best_loss_r,best_loss_l1=train_test_reg_mv(log_file, dataloaders, model, optimizer,checkpoint_format,data_params,train_params)
            new_df=pd.DataFrame({'model_name':[model_name],'features':[subset],'shift':[j],
                                'ckpt':[best_epo_path],'MSE':[best_loss],'RMSE':[best_loss_r],'L1':[best_loss_l1]})
            df=df.append(new_df)
            df.to_csv(os.path.join(direct_folder,out_file_name),index=False)
            display(df.tail())
...
def mv_transfer_trainer_v1(data_params,train_params,model_params):
    src_data=data_params['src_data']#<---
    tar_data=data_params['tar_data']#<--
    seq_size=data_params['seq_size']
    total_time_step=data_params['total_time_step']
...
#             if j % 30==0:
            print(f'....creating data for {i}th index, {L} length features at shift step {j}: \n{subset} ')
            # marker=f'tempo_shift{j}_feat{f}'
            feat_arr_src,label_arr_src, label_reg_arr_src=convert_npz_lagged_bkt_shift_mask(src_data,na_fill_type,seq_size,j, remove_feature_list, test_feature_list,domain_src,stu_num,marker,model_transform,save_data_flag)
            print(feat_arr_src.shape)
            print(label_reg_arr_src.shape)
            train_loader_src,val_loader_src,test_loader_src=get_mv_data_mask(feat_arr_src,label_arr_src, label_reg_arr_src,  batch_size,NUM_WORKER)

            feat_arr_tar,label_arr_tar, label_reg_arr_tar=convert_npz_lagged_bkt_shift_mask(tar_data,na_fill_type,seq_size,j, remove_feature_list, test_feature_list,domain_tar,stu_num,marker,model_transform,save_data_flag)
            print(feat_arr_tar.shape)
            print(label_reg_arr_tar.shape)
            train_loader_tar,val_loader_tar,test_loader_tar=get_mv_data_mask(feat_arr_tar,label_arr_tar, label_reg_arr_tar,  batch_size,NUM_WORKER)
            dataloaders={
            ...
            if model_name in ['lstm','LSTM','Lstm']:
                model=LSTM_TR(input_dim, hidden_dim, num_layers, output_dim,transfer_loss=transfer_loss_type).to(device)
                print(f'this is lstm model,you are training with {model_name} model')
            elif model_name in ['adaRNN','adarnn','AdaRNN']:
                print(f'this is adaRNN model,you are training with {model_name} model')
                model=AdaRNN(use_bottleneck=True, bottleneck_width=64, n_input=input_dim, n_hiddens=n_hiddens,  n_output=class_num, dropout=dropout, len_seq=tar_seqL, model_type=model_name,trans_loss=transfer_loss_type).to(device)
            else:
                print(f'this is transformer model,you are training with {model_name} model')
                model = Transformer_TR(input_dim, hidden_dim, output_dim, q, v, h, N,transfer_loss_type=transfer_loss_type, attention_size=attention_size,
                dropout=dropout, chunk_mode=chunk_mode, pe=pe).to(device)
