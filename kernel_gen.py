...
# kernels are functions that take in x1 and x2 to map the relationship between x1 and x2, it can be considered as a module or a nn layer in pytorch
# you can append kernels together like a list of modules in NN
"""
...
def generate_kernel(cat_kernel, bin_kernel, sqexp_kernel, cat_int_kernel, bin_int_kernel, covariate_missing_val):
    """
    Generate the additive kernel including id_covariate as categorical variable, no differentiation between with/without id_covariate
    ...
    :param covariate_missing_val:  list of dictionaries with indices from the covariate matrix with missing values
                            ...
                            # active dims is the id of covariate from auxilary covariate matrix defined in kernel_spec.py
...
def generate_kernel_batched(latent_dim, cat_kernel, bin_kernel, sqexp_kernel, cat_int_kernel, bin_int_kernel, covariate_missing_val,
                            id_covariate):
    """
    Generate two additive kernels. One with id covariate and the other without the id covariate with batch_size as latent_dim (not batched data)
    ...
    :param bin_int_kernel:  list of dictionaries with indices for the interaction kernel between a binary and continuous covariate
                            E.g.: [{'cont_covariate':1, 'bin_covariate':4}]
    # #{'cont_covariate':2, 'bin_covariate':1},{'cont_covariate':2, 'bin_covariate':3},{'cont_covariate':2, 'bin_covariate':5},{'cont_covariate':2, 'bin_covariate':7}
    :param covariate_missing_val:  list of dictionaries with indices from the covariate matrix with missing values
                            and their corresponding masks
                            E.g.: [{'covariate':0, 'mask': 3}]
    :param id_covariate:  index from the covariate matrix for the id covariate
