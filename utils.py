...
def create_npz_old(df_path,aux_data_path,enc_list,marker):
    """
    this func is to create the npz data from the engineered data and auxilary data and  masks as well
    """
    from sklearn.preprocessing import LabelEncoder
    ...
    df_eng['student_id']=df_eng['student_id'].astype(str)
    df_eng_aux=pd.merge(df_eng,aux_data,how='left',on='student_id')
    print(df_eng_aux.shape)
    if 'correct' in df_eng_aux.columns:
        df_aux_final=df_eng_aux[['event_time', 'LE_SCHOOL_ID','SPECIAL_ED', 'student_id','FREE_REDUCED_LUNCH', 'GIFTED_TALENTED', 'GRADE_LEVEL' ,'correct','score_rate']]
    else:
        df_aux_final=df_eng_aux[['event_time', 'LE_SCHOOL_ID','SPECIAL_ED', 'student_id','FREE_REDUCED_LUNCH', 'GIFTED_TALENTED', 'GRADE_LEVEL' ]]
        df_aux_final['correct']=np.nan
        df_aux_final['score_rate']=np.nan
    print(df_aux_final.shape)

    # df_aux_final.to_csv(f'data/k12/split_data/{marker}_outcome_labels.csv',index=False)
    df_aux_final_mask=np.where(df_aux_final.isna(),0,1)
    df_aux_final_mask_df=pd.DataFrame(df_aux_final_mask,columns=df_aux_final.columns)
    # df_aux_final_mask_df.to_csv(f'data/k12/split_data/{marker}_aux_final_mask_df.csv',index=False)

    df_data_final=df_eng_aux[[ 'seq_number', 'assessment_duration', 'bktcount','count',
        'quest_difficulty', 'item_difficulty', 'usmo_difficulty',
            'quest_ref_1', 'item_ref_1', 'usmoid_1',
        'question_type_1']]
    print(df_data_final.shape)
    # df_data_final.to_csv(f'data/k12/split_data/{marker}_data_readings.csv',index=False)
    df_data_final.isna().sum() #no missing values, we need the missing value version
    df_data_final_mask=np.where(df_data_final.isna(),0,1)
    df_data_final_mask_df=pd.DataFrame(df_data_final_mask,columns=df_data_final.columns)

    # df_data_final_mask_df.to_csv(f'data/k12/split_data/{marker}_data_final_mask_df.csv',index=False)
    print('data_readings/data_mask/outcome/outcome_mask shape:')
    print(df_data_final.shape,df_data_final_mask_df.shape,df_aux_final.shape,df_aux_final_mask_df.shape)
    df_data_final=df_data_final.fillna(0)
    data_readings=df_data_final.values
    ...
    labels=df_aux_final.fillna(0).values
    label_masks=df_aux_final_mask_df.values
    print(f'..saving npz data into data/k12/split_data/{marker}_data.npz...\n')
    np.savez(f'data/k12/split_data/{marker}_data.npz',data_readings=data_readings,outcome_attrib=labels,data_mask=data_masks,outcome_mask=label_masks)
    return df_data_final,df_data_final_mask_df,df_aux_final,df_aux_final_mask_df
...
def split_data_old1(data_path,row_num_list,save_dir,marker):
    data=np.load(data_path,allow_pickle=True)
    train_idx=row_num_list[0]
    val_idx=row_num_list[1]
    test_idx=row_num_list[2]
    pred_idx=row_num_list[3]
    gen_idx=row_num_list[4]
    train_readings=data['data_readings'][:train_idx]
    train_mask=data['data_mask'][:train_idx]
    train_outcome_attrib=data['outcome_attrib'][:train_idx]
    train_outcome_mask=data['outcome_mask'][:train_idx]
    print(f'..saving train data to {save_dir}/{marker}_train.npz...')
    np.savez(f'{save_dir}/{marker}_train.npz',data_readings=train_readings,
                data_mask=train_mask,outcome_attrib=train_outcome_attrib,outcome_mask=train_outcome_mask)
    val_readings=data['data_readings'][train_idx:train_idx+val_idx]
    val_mask=data['data_mask'][train_idx:train_idx+val_idx]
    val_outcome_attrib=data['outcome_attrib'][train_idx:train_idx+val_idx]
    val_outcome_mask=data['outcome_mask'][train_idx:train_idx+val_idx]
    print(f'..saving val data to {save_dir}/{marker}_val.npz...')
    np.savez(f'{save_dir}/{marker}_val.npz',data_readings=val_readings,
                data_mask=val_mask,outcome_attrib=val_outcome_attrib,outcome_mask=val_outcome_mask)
    test_start=train_idx+val_idx
    test_readings=data['data_readings'][test_start:test_start+test_idx]
    test_mask=data['data_mask'][test_start:test_start+test_idx]
    test_outcome_attrib=data['outcome_attrib'][test_start:test_start+test_idx]
    test_outcome_mask=data['outcome_mask'][test_start:test_start+test_idx]
    print(f'..saving test data to {save_dir}/{marker}_test.npz...')
    np.savez(f'{save_dir}/{marker}_test.npz',data_readings=test_readings,
                data_mask=test_mask,outcome_attrib=test_outcome_attrib,outcome_mask=test_outcome_mask)
    if pred_idx!=0:
        pred_start=test_start+test_idx
        pred_readings=data['data_readings'][pred_start:pred_start+pred_idx]
        pred_mask=data['data_mask'][pred_start:pred_start+pred_idx]
        pred_outcome_attrib=data['outcome_attrib'][pred_start:pred_start+pred_idx]
        pred_outcome_mask=data['outcome_mask'][pred_start:pred_start+pred_idx]
        print(f'..saving pred data to {save_dir}/{marker}_pred.npz...')
        np.savez(f'{save_dir}/{marker}_pred.npz',data_readings=pred_readings,
                    data_mask=pred_mask,outcome_attrib=pred_outcome_attrib,outcome_mask=pred_outcome_mask)
    else:
        pass
    if pred_idx!=0 and gen_idx!=0:
        gen_start=pred_start+pred_idx
        gen_readings=data['data_readings'][gen_start:gen_start+gen_idx]
        gen_mask=data['data_mask'][gen_start:gen_start+gen_idx]
        gen_outcome_attrib=data['outcome_attrib'][gen_start:gen_start+gen_idx]
        gen_outcome_mask=data['outcome_mask'][gen_start:gen_start+gen_idx]
        print(f'..saving gen data to {save_dir}/{marker}_gen.npz...')
        np.savez(f'{save_dir}/{marker}_gen.npz',data_readings=gen_readings,
                data_mask=gen_mask,outcome_attrib=gen_outcome_attrib,outcome_mask=gen_outcome_mask)
    else:
        pass



def split_data_old0(data,ratios,save_dir,marker):
    """
    this func is to generate train/val/test/pred/gen dataset for Physionet stype data 
    
    """
    train_ratio,val_ratio,test_ratio,gen_ratio,pred_ratio=ratios
    data_readings=data['data_readings']
    print(f'total len of data:{len(data_readings)}')
    train_idx=int(len(data_readings)*train_ratio)
    test_idx=int(len(data_readings)*test_ratio)
    val_idx=int(len(data_readings)*val_ratio)
    pred_idx=int(len(data_readings)*pred_ratio)
    gen_idx=int(len(data_readings)*gen_ratio)
    train_readings=data['data_readings'][:train_idx]
    train_mask=data['data_mask'][:train_idx]
    train_outcome_attrib=data['outcome_attrib'][:train_idx]
    train_outcome_mask=data['outcome_mask'][:train_idx]
